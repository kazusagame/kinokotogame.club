...
import { INIT_SKILL_LEVEL } from "@/features/decksim/data/skillData";
...
export function MainScenes({
  ...
  const sceneData = data.mainScenes?.[typeIndex] ?? {};
  const sceneCount = Object.keys(sceneData).length;
  const summaryData = summary.summaries.mainScenes?.[typeIndex] ?? {};
  ...
  return (
    <section className="pl-1">
      <h2 className="text-lg font-bold">主センバツ</h2>
      <div className="flex flex-col gap-6 mt-4 pl-0 sm:pl-2 md:pl-4">
        <RegisteredMainScenesBlock
          eventId={eventId}
          type={type}
          sceneData={sceneData}
...
function RegisteredMainScenesBlock({
  ...
  eventId: DeckSimulatorEventId;
  type: "攻援" | "守援";
  sceneData: DeckSimulatorData["mainScenes"]["attack" | "defense"];
  sceneCount: number;
  summaryData: DeckSimulatorResult["summaries"]["mainScenes"][
    | "attack"
    | "defense"];
...
function AddMainScenesBlock({
  ...
  const isSceneCountFull = sceneCount >= MAX_MAIN_GIRLS_NUM;
  return (
    <div className="pl-4">
      <button
        type="button"
              ...
              <div className="flex flex-col">
                <label className="label">
                  <span className="label-text">
                    <TextWithTooltip
                      displayText="ストラップ"
                      tipText="装着したストラップによって上昇する攻援力や守援力の数値を入力します。フラワーストラップの場合も％ではなく実際の効果値を入力します。"
                    />
                  </span>
                </label>
                <input
                  ...
                  min={0}
                  className="input input-md input-bordered max-w-24 md:w-24 text-right"
                  value={formatNumber(selected.strap)}
                  onChange={(e) =>
                    setSelected({
                      ...selected,
                      strap: e.target.value,
                    })
                  }
                />
              </div>
